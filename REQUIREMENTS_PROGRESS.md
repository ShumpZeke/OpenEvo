...
| Seed Forge | PARTIAL | `oe_max/search/seed_forge.py` | Builds a starting population from one seed with **no model requests**: literal and effort-dial scaling, then the same G0/G1 gates a mutated candidate faces. Measured on the shipped example: 7 valid distinct variants, scores 0.769–1.478 against the seed's 1.423, two of them better. PARTIAL because the variants that won did so by running the search harder — trading local compute for score, not a better algorithm — and because nothing yet seeds a run from the forge |
