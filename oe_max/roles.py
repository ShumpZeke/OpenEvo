...
_PREFERENCES: Dict[Role, Chain] = {
    # Ordered by which reasoning-class route answers every time. Note
    # *reasoning-class*: the 120B is faster than everything here and is
    # deliberately not eligible, because a mutation generated by the cheap route
    # is a worse mutation -- see the invariant in tests/oe_max/test_roles.py.
    # Speed that costs mutation quality is not efficiency.
    #
    # kimi-k3 is the operator's preference and led this chain until the
    # allowance was measured on 2026-08-29: about ten calls over twenty minutes
    # exhausted it, after which it refused with HTTP 429 in 0.2s -- an instant
    # refusal, not a queue -- while nemotron-super, -ultra and -nano all
    # returned 200 in the same minute. A per-model allowance, not our limiter
    # and not the account.
    #
    # Leading with it worked: failover served on attempt 2, verified. But it
    # spent one of four attempts on a refusal for every request. So the flagship
    # leads -- 3.13s, and it answered every time it was asked -- and kimi sits
    # second, where it costs nothing while exhausted and is used the moment its
    # allowance returns. When kimi is serving it is roughly twice as quick, so
    # this order is worth revisiting if the allowance changes.
    #
    # To run kimi and nothing else, pick it in single-model mode. That is what
    # the mode is for, and it pins rather than falling back.
    Role.REASONER: [
    ...
    # Same reasoning as REASONER above, and the same constraint: the 120B is not
    # eligible to lead a code role either. kimi-k3 is the specialist chosen
    # *for* code and re-measured at 1.59s, but its allowance runs out in about
    # ten calls, so the flagship leads and kimi sits second. deepseek-v4-flash
    # serves at 15.8s, well below both.
    Role.CODER: [
        ("nvidia_nim", "nemotron_ultra_253b"),
        ("nvidia_nim", "kimi_k3"),
