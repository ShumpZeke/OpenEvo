...
## A per-model allowance looks exactly like a broken provider

Third variant of the 429 problem, and the one that most looks like your own
fault.

Measured on NVIDIA NIM, 2026-08-29. `moonshotai/kimi-k3` served in 1.59 s, was
promoted to lead the reasoner chain, and roughly **ten calls over twenty
minutes** exhausted it. After that it refused with HTTP 429 in **0.2 s** —
instant, not a queue — for at least five minutes.

The instinct is that the limiter is misconfigured, or the account is throttled,
or the key is wrong. It was none of those, and one probe settles it: ask a
*different* model on the same account in the same minute.

    moonshotai/kimi-k3                  429 in 0.20s
    nvidia/nemotron-3-super-120b-a12b   200 in 0.56s
    nvidia/nemotron-3-ultra-550b-a55b   200 in 3.13s
    nvidia/nemotron-3-nano-30b-a3b      200 in 0.45s

Three routes answering while one refuses is a per-model allowance. It is not the
account, and it is not `RateLimiter` — ours governs *our* attempt starts, and
cannot cause a refusal that arrives in 200 ms.

### What it means for chain order

A model that serves beautifully in a probe and has a ten-call allowance is not a
chain head. Leading with it worked — failover served on attempt 2, verified —
but spent one attempt of four on a refusal for **every** request, leaving the
role one hiccup from having none left.

So measure the allowance, not just the latency. A latency table will tell you to
promote exactly the model you should not.

### The reorder that the test suite refused

Worth recording because the fix was wrong in an instructive way. The obvious
replacement head was `nemotron-3-super-120b-a12b`: fastest route measured on any
provider, and it answered every time.

`test_the_reasoning_roles_do_not_lead_with_the_fast_route` failed, correctly.
The 120B is the *cheap* route — it leads the judge and fast roles for that
reason — and **a mutation generated by the cheap route is a worse mutation**.
Speed that costs mutation quality is not efficiency, and the invariant exists
because reordering for latency is the tempting mistake.

The head has to be reasoning-class *and* reliable, which is the flagship.

## Chains are built at broker startup

Editing `oe_max/roles.py` and re-running a request changes nothing: the router
composes its chains when the broker starts. A config change needs a restart, and
the symptom of forgetting is a request served by the model you just demoted,
with an `attempt` number that makes sense only under the old order.
