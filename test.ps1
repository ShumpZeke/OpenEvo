...
& $py -m pytest tests\ -q -m "not slow" --ignore=tests\evolution --ignore=tests\oe_max --ignore=tests\brain
...
Write-Host "`n=== BrainPort (OpenCode brain, worker, plugin contract) ==="
& $py -m pytest tests\brain -q
if ($LASTEXITCODE -ne 0) { $rc = 1 }
